...
export default function AdminDriverDetail() {
  ...
  const handleDocUpload = async (e) => {
    ...
    try {
      ...
      await driversAPI.adminUploadDoc(id, uploadTipo, formData);
      toast.success(`${uploadTipo.toUpperCase()} enviado!`);
      await loadData();
    } catch (err) {
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="card">
        <h3 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
          <FileText className="w-4 h-4 text-brand-600" /> Documentos
        </h3>

        {/* Hidden file input */}
        <input type="file" ref={docInputRef} onChange={handleDocUpload} accept="image/*,application/pdf" className="hidden" />

        {/* Upload buttons por tipo */}
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-4">
          {[
            { tipo: 'cnh', label: 'CNH', url: driver.cnh_url },
            { tipo: 'comprovante', label: 'Comprovante', url: driver.comprovante_url },
            { tipo: 'selfie', label: 'Selfie', url: driver.selfie_url },
            { tipo: 'contrato', label: 'Contrato', url: driver.contrato_url },
            { tipo: 'outro', label: 'Outro Doc', url: null },
          ].map(doc => (
            <div key={doc.tipo} className={`rounded-lg border p-3 text-center ${doc.url ? 'border-green-200 bg-green-50' : 'border-gray-200 bg-gray-50'}`}>
              <p className="text-xs font-medium text-gray-700 mb-2">{doc.label}</p>
              <div className="flex items-center justify-center gap-1.5">
                {doc.url && (
                  <button onClick={() => setPreviewUrl(doc.url)} className="text-xs bg-white border border-gray-200 text-brand-600 px-2 py-1 rounded hover:bg-brand-50 flex items-center gap-1">
                ...
                <button
                  ...
                  className={`text-xs px-2 py-1 rounded flex items-center gap-1 ${
                    doc.url ? 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-100' : 'bg-brand-600 text-white hover:bg-brand-700'
                  }`}
                >
                  <Upload className="w-3 h-3" /> {doc.url ? 'Trocar' : 'Enviar'}
                </button>
              </div>
              {doc.url && <p className="text-[10px] text-green-600 mt-1">✓ Enviado</p>}
            </div>
          ))}
        </div>
        ...
        {driver.documents && driver.documents.length > 0 && (
          <div className="border-t border-gray-100 pt-3">
            <p className="text-xs font-medium text-gray-500 mb-2">Histórico de envios</p>
